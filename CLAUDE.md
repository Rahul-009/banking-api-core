...
- `/api-docs` — interactive Swagger UI (public, no auth gate — intentional for a portfolio demo). Spec is generated by `swagger-jsdoc` from `@swagger` YAML blocks in `src/routes/*.routes.js`, config in `src/config/swagger.js`/`swagger.schemas.js`. **Gotcha**: `@swagger` must be the only tag in its own `/** */` comment block — a block that also has `@route`/`@desc`/`@access` (or any other custom tag) before `@swagger` causes swagger-jsdoc to silently extract zero paths from that block, no error thrown. Put `@swagger` in a second, separate comment block immediately after the `@route/@desc/@access` one.
